...
function WatchedValue({ name }: { name: string }) {
  ...
  const serialized = value instanceof File ? `FILE:${value.name}` : JSON.stringify(value ?? null);

  return <output data-testid={`${name}-value`}>{serialized}</output>;
}

// Wrapper component for testing with react-hook-form
function TestFormWrapper({ field, defaultValue }: { field: MetaField; defaultValue?: unknown }) {
  const form = useForm({
    defaultValues: {
...
describe("FormFieldRenderer", () => {
  ...
  describe("File and Image Fields", () => {
    it("updates RHF state when selecting a file field value", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId("contract_file-value")).toHaveTextContent(
