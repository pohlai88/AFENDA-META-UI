...
function getArrayValidationError(
...
export function DynamicForm({
  ...
  const getFieldError = React.useCallback(
    (field: LeafFieldConfig, value: unknown): string | null => {
      const isEmpty =
        value == null ||
      ...
      if (
        ...
        typeof value === "string" &&
        !field.validate.pattern.test(value)
      ) {
      ...
      const validateArrayFields = (
        ...
        nodes.forEach((node) => {
          ...
          if (isFieldArrayConfig(node)) {
            ...
            const items = Array.isArray(source[node.name])
  ...
  const renderNodes: (nodes: FieldConfig[], path?: string[]) => React.ReactNode[] =
    React.useCallback(
      (nodes: FieldConfig[], path: string[] = []): React.ReactNode[] => {
