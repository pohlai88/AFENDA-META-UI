...
opsRouter.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const entityType = typeof req.query.entityType === "string" ? req.query.entityType : undefined;
    const tenantId =
      typeof req.query.tenantId === "string" ? parseInteger(req.query.tenantId, NaN) : undefined;
    const violationType =
      typeof req.query.violationType === "string" ? req.query.violationType : undefined;
    const dateFrom = parseDate(req.query.dateFrom);
    const dateTo = parseDate(req.query.dateTo);
    ...
    whereClause = withOptional(
      whereClause,
      policyId ? eq(mutationPolicyViolations.policyId, policyId) : undefined
    );
    whereClause = withOptional(
      whereClause,
      entityType ? eq(mutationPolicyViolations.entityType, entityType) : undefined
    );
    whereClause = withOptional(
      whereClause,
      tenantId != null && Number.isFinite(tenantId)
        ? eq(mutationPolicyViolations.tenantId, tenantId)
        : undefined
    );
    whereClause = withOptional(
      whereClause,
      violationType ? eq(mutationPolicyViolations.violationType, violationType) : undefined
    );
    whereClause = withOptional(
      whereClause,
      dateFrom ? gte(mutationPolicyViolations.createdAt, dateFrom) : undefined
    );
    whereClause = withOptional(
      whereClause,
      dateTo ? lte(mutationPolicyViolations.createdAt, dateTo) : undefined
    );

    const listBaseQuery = db
      .select({
        id: mutationPolicyViolations.id,
        policyId: mutationPolicyViolations.policyId,
        entityType: mutationPolicyViolations.entityType,
        entityId: mutationPolicyViolations.entityId,
        tenantId: mutationPolicyViolations.tenantId,
        operation: mutationPolicyViolations.operation,
        violationType: mutationPolicyViolations.violationType,
        details: mutationPolicyViolations.details,
        createdAt: mutationPolicyViolations.createdAt,
      })
      .from(mutationPolicyViolations);
    ...
    const [rows, countRows] = await Promise.all([
      listQuery.orderBy(desc(mutationPolicyViolations.createdAt)).limit(limit).offset(offset),
      countQuery,
    ]);
