...
} from "../mutation-policy-runtime.js";
...
describe("mutation policy runtime helpers", () => {
  ...
  it("blocks configured direct mutations for event-only models", () => {
    expect(
      isDirectMutationAllowed({ model: "sales_order", operation: "update", policies })
    ).toEqual(
    ...
    expect(
      isDirectMutationAllowed({ model: "sales_order", operation: "create", policies })
