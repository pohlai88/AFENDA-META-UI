 ...
 *   normalize → compile-pipeline (invariants, cross-invariants, mutation policies, transitions, events) → emit → write
 *
 * Usage:
...
import { emitSqlSegments, TRUTH_SQL_BUNDLE_HEADER } from "./emitter.js";
...
const PRIMITIVES_PATH = resolve(CURRENT_DIR, "sql/truth-runtime-primitives.sql");
const SUPPLEMENT_PATH = resolve(CURRENT_DIR, "sql/truth-supplemental-triggers.sql");
...
function run(): void {
  ...
  // 2. Run all compiler stages (same order as `buildOrderedSqlSegments`)
  const segments = buildOrderedSqlSegments(normalized);

  // 3. Emit deterministic SQL bundle (primitives → compiler → supplemental hand-SQL)
  const primitives = readFileSync(PRIMITIVES_PATH, "utf-8").trimEnd();
  const supplement = readFileSync(SUPPLEMENT_PATH, "utf-8").trimEnd();
  const timestamp = new Date().toISOString();
  const generated = [
    TRUTH_SQL_BUNDLE_HEADER,
    `-- Generated at: ${timestamp}`,
    "",
    primitives,
    "",
    emitSqlSegments(segments),
    "",
    supplement,
    "",
  ].join("\n");
