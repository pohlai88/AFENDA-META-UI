> **Navigation:** [Package README](../README.md) · [Architecture](../ARCHITECTURE.md) — long-form CLI, monitoring, and troubleshooting below.
...
The Graph Validation System provides automated checks for referential integrity across Foreign Key relationships in the ERP Postgres schemas (default: `core`, `hr`, `sales`, `inventory`, `accounting`, `purchasing`, `reference`, `security`). It detects orphaned records, validates tenant isolation, checks FK index coverage, and generates a comprehensive 0-100 health score with actionable recommendations.

**Implementation baseline & GraphQL boundary:** see [SKILLS.md](./SKILLS.md) and [GRAPHQL-BOUNDARY.md](./GRAPHQL-BOUNDARY.md).
...
- Adjunct lanes (Squawk, FK drift, schema manifest): see [ADJUNCTS.md](./ADJUNCTS.md)
- Graded runtime policy (`decision`, TTL, staleness): see [POLICY_LIFECYCLE.md](./POLICY_LIFECYCLE.md)
...
By default the CLI loads FK metadata from all ERP schemas listed above. Narrow scope with repeated `--schema=` (example: sales + HR only):
...
# JSON report for CI/CD or monitoring (v1 contract: reportVersion, policy, healthScore, …)
pnpm --filter @afenda/db graph-validation report --format=json > validation-report.json

# Include OSS-inspired adjuncts (Squawk on migrations if installed, optional FK drift baseline)
pnpm --filter @afenda/db graph-validation report --format=json --adjuncts --repo-root=. > validation-report.json
```

Validate a saved report file against the strict v1 parser (required top-level sections: `schemasCovered`, `indexCoverage`, `orphans`, `tenantLeaks`, `catalog`; optional `adjuncts.checks` validated when present):
...
1. Bump `GRAPH_VALIDATION_REPORT_VERSION` in [`types.ts`](../types.ts) when the JSON shape breaks consumers.
2. Update `parseGraphValidationReportJson` in [`policy-from-report.ts`](../policy-from-report.ts), golden fixtures under [`__test__/fixtures/`](../__test__/fixtures/), and optional [`report-v1.schema.json`](../report-v1.schema.json).
3. Document migration in this README (what changed, dual-parse window if needed).
4. Regenerate any saved CI baselines (`fk-catalog` fingerprint) when FK topology changes.
...
├── fk-catalog.ts             # FK metadata extraction (multi-schema)
...
├── graph-validation-docs/    # ADJUNCTS, OPERATIONS (this doc), SKILLS, …
├── baselines/                # FK catalog baseline README (export JSON when ready)
├── README.md                 # Package overview (template)
...
### Phase 3: Automated health check & production truth (fail-closed)

**Workflow:** `.github/workflows/graph-validation.yml` — job `graph-validation-health`.

#### Triggers & environment

| Item | Detail |
| ---- | ------ |
| **When** | Schedule `0 2 * * *` (UTC), `workflow_dispatch`, or push to **protected** refs (`github.ref_protected`) |
| **GitHub Environment** | `production` (use for secrets + optional approval gates) |
| **Timeout** | 20 minutes |
| **Live DB** | Requires **`DATABASE_URL`** available to the job (typically via environment secrets) |

If `DATABASE_URL` is missing, the job **skips** DB steps (no false green on an empty secret).

#### Runner stack & system tools (best practice)

| Layer | Version / note |
| ----- | -------------- |
| **OS** | `ubuntu-latest` (GitHub-hosted) |
| **Node.js** | `22.x` (aligned with workflow `NODE_VERSION`) |
| **Package manager** | **pnpm** — version pinned in workflow (`PNPM_VERSION`, e.g. `10.33.0`) via Corepack |
| **JSON / gates** | **`jq`** — preinstalled on GitHub-hosted Ubuntu; used for **fail-closed** assertions on `validation-report.json` (do not rely on parsing CLI stdout for gates) |

**Pinning:** Keep workflow Node/pnpm versions aligned with root `package.json` (`packageManager`, `engines` if present) so local `pnpm install` / `tsx` behavior matches CI.

#### Monorepo dependencies (what the workflow actually runs)

| Step | Command / package surface |
| ---- | --------------------------- |
| Install | `pnpm install --frozen-lockfile` (workspace root) |
| Tests (other job) | `@afenda/db` scripts: `db:test:graph-validation`, coverage gates |
| Health CLI | `pnpm --filter @afenda/db graph-validation` → `packages/db/src/graph-validation/runner.ts` via **`tsx`** (see `packages/db/package.json` `graph-validation` script) |
| Artifact contract | `pnpm --filter @afenda/db graph-validation:validate-artifact` → `validate-artifact.ts` (strict v1 parse) |
| Adjunct enforcement | `node tools/ci-gate/graph-validation/check-adjuncts.mjs` (optional **`CI_GRAPH_ADJUNCTS_STRICT`**, **`vars.*`**) |

Runtime npm deps for the CLI come from **`@afenda/db`**: `drizzle-orm`, `pg`, `zod`, etc. (see `packages/db/package.json`). No extra global tools beyond **Node + pnpm + jq**.

#### Single source of truth: `validation-report.json`

Production gates use **one** machine-readable artifact after a full `report --format=json` run:

1. **`graph-validation:validate-artifact`** — schema/contract parse (`parseGraphValidationReportJson`).
2. **`check-adjuncts.mjs`** — adjunct policy (Squawk / baseline / manifest rules as configured).
3. **`Assert production truth (v1 JSON, fail-closed)`** — `jq -e` on the same file:
   - `.policy.isSecurityBlocking == false`
   - `.healthScore.overall >= MIN_HEALTH_SCORE` (workflow env, default **70**)
   - `.tenantLeaks.isSecure == true`
   - `.orphans.byPriority.P0 == 0` **and** `.orphans.byPriority.P1 == 0` (matches CLI “critical” orphan tier)

**Operational rule:** If any step fails, the job fails **immediately** — no `continue-on-error` on these gates, and **no** grep-based parsing of human CLI output for pass/fail.

#### Artifacts produced (health job)

| File | Role |
| ---- | ---- |
| `health-check.txt` | Human overview from `graph-validation health` (operator-readable; **not** the compliance gate) |
| **`validation-report.json`** | **Authoritative** — policy, healthScore, orphans, tenantLeaks, catalog, optional adjuncts |
| `fk-catalog.json` | Exported FK catalog for drift baselines / review |

Optional adjunct output (e.g. schema manifest) depends on **`GRAPH_VALIDATION_SCHEMA_MANIFEST_DIR`** and env vars documented in [ADJUNCTS.md](./ADJUNCTS.md).

#### Workflow thresholds (summary env for dashboards)

- **`MIN_HEALTH_SCORE`** / **`ALERT_HEALTH_SCORE`** — drive status labels in the workflow summary after gates pass.
- **`P0_STATUS` / `TENANT_STATUS`** in the summary reflect **JSON truth** (PASS / SECURE) once assertions succeed.

#### GitHub setup checklist

1. **Secret:** `DATABASE_URL` on the `production` environment (or repository secret if you do not use environments).
2. **Optional:** `vars.CI_GRAPH_ADJUNCTS_STRICT` for adjunct fail-closed behavior.
3. **Network:** Runner must reach Postgres (Neon / RDS allowlists, firewall, TLS).
4. **Least privilege:** DB role used in CI should be **read-only** where possible; avoid superuser for validation-only jobs.

**Example workflow summary (after success):**
...
### Generated Reports
- Health check (text), validation-report.json (source of truth), FK catalog export
### Artifacts
...
- Docs: [Package README](../README.md) and [this docs folder](./)
