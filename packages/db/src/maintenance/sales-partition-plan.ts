...
export type SalesPartitionPlan = {
  generatedAt: string;
  schemaName: string;
  parentTable: string;
  years: number[];
...
export type SalesPartitionPlanOptions = {
  now?: Date;
  schemaName?: string;
  parentTable?: string;
  yearsAhead?: number;
...
export function buildSalesPartitionPlan(
  options: SalesPartitionPlanOptions = {}
): SalesPartitionPlan {
  const now = options.now ?? new Date();
  const schemaName = options.schemaName ?? "sales";
  const parentTable = options.parentTable ?? "sales_orders";
...
BEGIN
  ...
  EXECUTE format(
    'CREATE TABLE %I.%I (LIKE %I.%I INCLUDING ALL) PARTITION BY RANGE (order_date)',
    '${schemaName}',
    '${parentTable}',
    '${schemaName}',
    '${legacyParent}'
