 ...
 */
function selectPartnerPareto(partners: string[], index: number): string {
...
function generateOrder(
  ...
  const partnerId = selectPartnerPareto(config.partners, index);
  ...
  const lineData = Array.from({ length: numLines }, (_, lineIndex) => {
  ...
  const lines = lineData.map((line, lineIndex) => ({
    ...
    discountSource: line.discount > 0 ? "manual" as const : "none" as const,
