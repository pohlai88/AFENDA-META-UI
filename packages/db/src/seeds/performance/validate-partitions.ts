 ...
 *
 * Run with: pnpm --filter @afenda/db tsx src/seeds/performance/validate-partitions.ts
 */
...
/** Rows from pg_catalog partition listing query */
interface PartitionListRow {
...
async function listPartitions(schemaName: string, tableName: string): Promise<PartitionInfo[]> {
  const result = await pool.query<PartitionListRow>(
    `
    SELECT
 ...
 */
async function explainQuery(query: string, params: unknown[] = []): Promise<string> {
  const result = await pool.query<ExplainPlanRow>(`EXPLAIN (ANALYZE, BUFFERS) ${query}`, params);
