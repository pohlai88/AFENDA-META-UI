...
/** Minimum speedup (pruned vs full scan) we expect when partitioning is effective */
const EXPECTED_PARTITION_SPEEDUP = 3;
...
describe.skipIf(skipDbBenchmarks)("Partition Performance Benchmarks", () => {
  let tenantId: number;
  let hasSeededSalesOrders = false;
  ...
  beforeAll(async () => {
    ...
    if (!result[0]) {
      console.warn("Skipping Partition Performance Benchmarks: no sales orders found in database.");
      return;
    }

    hasSeededSalesOrders = true;
    tenantId = result[0].tenantId;
  });

  it("should demonstrate partition pruning benefits", async () => {
    if (!hasSeededSalesOrders) return;
    ...
    expect(comparison.speedup).toBeGreaterThanOrEqual(EXPECTED_PARTITION_SPEEDUP);
  });
...
describe.skipIf(skipDbBenchmarks)("Index Performance Benchmarks", () => {
  ...
  beforeAll(async () => {
    ...
    if (!result[0]) {
      console.warn("Skipping Index Performance Benchmarks: no sales orders found in database.");
      return;
    }
...
describe.skipIf(skipDbBenchmarks)("Join Performance Benchmarks", () => {
  ...
  beforeAll(async () => {
    ...
    if (!result[0]) {
      console.warn("Skipping Join Performance Benchmarks: no sales orders found in database.");
      return;
    }
...
describe.skipIf(skipDbBenchmarks)("Metadata Override Resolution Benchmarks", () => {
  let tenantId: string;
  let hasTenantScopedMetadataOverrides = false;
  ...
  beforeAll(async () => {
    ...
    if (!result[0]?.tenantId) {
      console.warn(
        "Skipping Metadata Override Resolution Benchmarks: no tenant-scoped metadata overrides found."
      );
      return;
    }

    hasTenantScopedMetadataOverrides = true;
    tenantId = result[0].tenantId;
  });

  it("should benchmark single entity metadata resolution", async () => {
    if (!hasTenantScopedMetadataOverrides) return;
