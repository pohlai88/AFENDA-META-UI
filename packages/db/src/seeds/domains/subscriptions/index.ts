...
export async function seedSubscriptionsPhase9(
  ...
  await tx
    .insert(subscriptionStatuses)
    .values([
      { code: "draft", name: "Draft", sortOrder: 10 },
      { code: "active", name: "Active", sortOrder: 20 },
      { code: "paused", name: "Paused", sortOrder: 30 },
      { code: "past_due", name: "Past due", sortOrder: 40 },
      { code: "cancelled", name: "Cancelled", sortOrder: 50 },
      { code: "expired", name: "Expired", sortOrder: 60 },
    ])
    .onConflictDoNothing()
    .execute();
  ...
  const [vipPricelist] = await tx
    .select({ currencyId: pricelists.currencyId })
    .from(pricelists)
    .where(eq(pricelists.id, SEED_IDS.pricelistUsdVip))
    .limit(1);
    ...
    .values([
      {
        ...
        arr,
        truthRevision: 1,
        pricingLockedAt: anchor,
        pricingSnapshot: {},
        currencyId: vipPricelist?.currencyId ?? null,
        closeReasonId: null,
...
export async function validateSubscriptionsPhase9Invariants(tx: Tx): Promise<void> {
  ...
  const [closeReason] = await tx
    .select({
      code: subscriptionCloseReasons.code,
      isChurn: subscriptionCloseReasons.isChurn,
    })
    .from(subscriptionCloseReasons)
    .where(eq(subscriptionCloseReasons.id, SEED_IDS.subscriptionCloseReasonBudget));

  if (!closeReason?.isChurn) {
    throw new Error("Subscription close reason invariant: seeded churn reason must have isChurn=true");
  }

  const statusRows = await tx.select({ code: subscriptionStatuses.code }).from(subscriptionStatuses);
  const codes = new Set(statusRows.map((r) => r.code));
  for (const required of ["draft", "active", "paused", "past_due", "cancelled", "expired"] as const) {
    if (!codes.has(required)) {
      throw new Error(`Subscription status lookup mismatch: missing code ${required}`);
    }
  }
