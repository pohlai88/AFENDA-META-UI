...
export async function ensureDefaultTenant(tx: Tx): Promise<number> {
  const existing = await tx
    ...
    .from(tenants)
    .where(sql`lower(${tenants.tenantCode}) = lower(${DEFAULT_TENANT_CODE})`)
    .limit(1);
  ...
  try {
    ...
    const conflicted = await tx
      ...
      .where(sql`lower(${tenants.tenantCode}) = lower(${DEFAULT_TENANT_CODE})`)
      .limit(1);
    if (conflicted.length > 0) return conflicted[0].tenantId;
    throw error;
  }
}
...
export async function ensureSystemUser(tx: Tx, tenantId: number): Promise<number> {
  try {
    ...
    // Local DB compatibility fallback: some environments expose security.users.tenant_id.
    if (code !== "42703") throw error;

    const existingRaw = await tx.execute(
      sql`
        select "userId"
        from "security"."users"
        where "tenant_id" = ${tenantId}
          and lower("email") = lower(${DEFAULT_SYSTEM_USER_EMAIL})
    ...
    const existingUserId =
      (existingRaw as { rows?: Array<{ userId?: number; user_id?: number }> }).rows?.[0]?.userId ??
      (existingRaw as { rows?: Array<{ userId?: number; user_id?: number }> }).rows?.[0]?.user_id;
    if (typeof existingUserId === "number") {
      return existingUserId;
    }

    const insertedRaw = await tx.execute(
      sql`
    ...
    const insertedUserId =
      (insertedRaw as { rows?: Array<{ userId?: number; user_id?: number }> }).rows?.[0]?.userId ??
      (insertedRaw as { rows?: Array<{ userId?: number; user_id?: number }> }).rows?.[0]?.user_id;
    return typeof insertedUserId === "number" ? insertedUserId : SYSTEM_ACTOR_ID;
  }
}
