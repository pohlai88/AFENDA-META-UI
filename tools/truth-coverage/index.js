 ...
 */
export function calculateTruthCoverage(rootDir) {
 ...
 */
function scanForDirectDbWrites(rootDir) {
 ...
 */
export function formatCoverageReport(metrics) {
    const lines = [];
    ...
    lines.push(`State Machine Coverage: ${metrics.stateMachinesCovered}/${metrics.stateMachinesTotal} (${metrics.stateMachineCoveragePercent.toFixed(1)}%)`);
    lines.push("");
 ...
 */
export function enforceCoverageThresholds(metrics, thresholds) {
    const errors = [];
    if (metrics.invariantCoveragePercent < thresholds.minInvariantCoverage) {
        errors.push(`Invariant coverage ${metrics.invariantCoveragePercent.toFixed(1)}% is below threshold ${thresholds.minInvariantCoverage}%`);
    }
    if (metrics.stateMachineCoveragePercent < thresholds.minStateMachineCoverage) {
        errors.push(`State machine coverage ${metrics.stateMachineCoveragePercent.toFixed(1)}% is below threshold ${thresholds.minStateMachineCoverage}%`);
    }
