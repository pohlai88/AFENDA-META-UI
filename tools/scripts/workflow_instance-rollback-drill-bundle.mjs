...
const context = resolveNonSalesGovernanceContext({ root: ROOT, args: [...args], aggregate: "workflow_instance" });
...
const bundle = [
  ...
  {
    title: "Workflow instance replay concurrency validation",
    command: "pnpm",
    args: ["--filter", "@afenda/api", "test", "--", "--run", "src/events/__test__/workflow-replay-concurrency-fixture.test.ts"],
  },
  {
    title: "Checkpoint drift diagnostics",
    command: "pnpm",
    args: ["--filter", "@afenda/api", "test", "--", "--run", "src/events/__test__/projectionRuntime.test.ts"],
  },
];
...
const lines = [];
lines.push("# Workflow Instance Rollback Drill Evidence");
lines.push("");
lines.push(`- Generated at (UTC): ${finishedAt}`);
lines.push("- Aggregate: workflow_instance");
