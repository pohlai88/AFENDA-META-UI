...
const context = resolveNonSalesGovernanceContext({ root: ROOT, args: [...args], aggregate: "tenant" });
...
const bundle = [
  ...
  {
    title: "Cross-context cascade validation (tenant -> workflow)",
    command: "pnpm",
    args: ["--filter", "@afenda/api", "test", "--", "--run", "src/events/__test__/cross-context-drift-fixture.test.ts"],
  },
  {
    title: "Platform replay parity check",
    command: "pnpm",
    args: ["--filter", "@afenda/api", "test", "--", "--run", "src/routes/__test__/tenant.route.test.ts"],
  },
];
...
const lines = [];
lines.push("# Tenant Rollback Drill Evidence");
lines.push("");
lines.push(`- Generated at (UTC): ${finishedAt}`);
lines.push("- Aggregate: tenant");
